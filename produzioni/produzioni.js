...
function renderProductions(productions) {
  ...
  productions.forEach((production, index) => {
    ...
    const productionDetails = `
      <div class="production-details" id="${production.Nome}">
...
function toggleProductionDetails(productionName) {
  ...
  // Chiudi tutte le voci aperte prima di aprire quella selezionata
  const allDetails = document.querySelectorAll('.production-details');
  allDetails.forEach(detail => {
    if (detail !== productionDetails && detail.classList.contains('open')) {
      detail.classList.remove('open'); // Chiudi le voci non selezionate
    }
  });

  // Alterna l'apertura/chiusura per la voce selezionata
