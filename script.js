...
dropdowns.forEach(link => {
    link.addEventListener('click', function (e) {
        ...
        if (window.innerWidth <= 768) {
            e.preventDefault(); // Impedisce il comportamento normale del link

            const submenu = this.nextElementSibling; // Seleziona il sottomenu associato

            // Se il sottomenu è già aperto, lo chiude
            if (submenu.classList.contains('open')) {
                submenu.classList.remove('open');
            } else {
                // Chiude tutti i sottomenu
                const allSubmenus = document.querySelectorAll('.navbar .dropdown .submenu');
                allSubmenus.forEach(sm => sm.classList.remove('open'));
