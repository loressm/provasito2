...
function renderMembers(members) {
  ...
  members.forEach((member, index) => {
    ...
    memberDiv.innerHTML = `
      <h3>${member.Nome}</h3>
      <div class="member-details" id="${detailsId}">
...
function toggleMemberDetails(name) {
  const allDetails = document.querySelectorAll('.member-details');
  allDetails.forEach(detail => {
    if (!detail.closest('.member').querySelector('h3').textContent.includes(name)) {
      detail.style.display = 'none';
    }
  });

  const clicked = Array.from(document.querySelectorAll('.member')).find(m => 
    m.querySelector('h3')?.textContent.includes(name)
  );

  const detailDiv = clicked.querySelector('.member-details');
